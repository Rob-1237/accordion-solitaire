...
// Save user profile (displayName, email) to Firestore
export const saveUserProfile = async (userId: string, profile: { displayName: string; email: string }) => {
    if (!userId) {
        console.error('No user ID provided for saving user profile.');
        return;
    }
    try {
